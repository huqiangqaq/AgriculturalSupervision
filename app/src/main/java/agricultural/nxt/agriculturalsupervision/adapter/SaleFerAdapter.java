...
import agricultural.nxt.agriculturalsupervision.Activity.sales.SaleFerAddActivity;
...
/**
 * Created by huqiang on 2017/2/4 13:20.
 */

public class SaleFerAdapter extends RecyclerView.Adapter<SaleFerAdapter.ViewHolder> {
    private List<SaleFer.ListBean> list;
    private Context mContext;
    private ProgressDialog loadingDialog;
    private String type;

    public SaleFerAdapter(List<SaleFer.ListBean> list, Context mContext, String type) {
        this.list = list;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_sale_fer,parent,false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(ViewHolder holder, int position) {
        SaleFer.ListBean sale  = list.get(position);
        holder.id.setText(sale.getSoh().getId());
        holder.vcorgname.setText(sale.getOwner().getVcorgname());
        holder.vcvarietyname.setText(sale.getFertilizerbatch().getFertilizer().getVcfertilizename());
...
//        String sb = format.format(date);
    ...
    private void update(ViewHolder holder, int position) {
        Intent intent = new Intent(mContext, SaleFerAddActivity.class);
        SaleFer.ListBean listBean = list.get(position);
        intent.putExtra("salefer",listBean);
    ...
    private void delete(ViewHolder holder, int position) {
        new AlertDialog.Builder(mContext)
                ...
                .setPositiveButton("确定", (dialog, which) -> {
                    ...
                    OkhttpHelper.Get(Constants.SALE_FER_DEL + id, new OkhttpHelper.GetCallBack() {
                        @Override
