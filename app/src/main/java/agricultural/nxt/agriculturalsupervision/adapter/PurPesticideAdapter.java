...
/**
 * Created by huqiang on 2017/1/19 11:30.
 */
...
public class PurPesticideAdapter extends RecyclerView.Adapter<PurPesticideAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        PurPesticide.ListBean purchase = list.get(position);
        holder.id.setText(purchase.getPoh().getId());
        holder.vcorgname.setText(purchase.getOwner().getVcorgname());
        holder.vcvarietyname.setText(purchase.getPesticide().getVcpesticidename());
    ...
    private void update(ViewHolder holder, int position) {
        Intent intent = new Intent(mContext, PurPestAddActivity.class);
        PurPesticide.ListBean listBean = list.get(position);
        intent.putExtra("purPest",listBean);
    ...
    private void delete(ViewHolder holder, int position) {
        ...
        new AlertDialog.Builder(mContext)
                ...
                .setPositiveButton("确定", (dialog, which) -> {
                    ...
                    OkhttpHelper.Get(Constants.PUR_PESTICIDE_DEL + id, new OkhttpHelper.GetCallBack() {
                        @Override
