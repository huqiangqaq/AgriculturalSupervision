...
/**
 * Created by huqiang on 2017/1/18 10:14.
 */
...
public class PurchaseAdapter extends RecyclerView.Adapter<PurchaseAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        Purchase.ListBean purchase = list.get(position);
        holder.id.setText(purchase.getPohdId().getId());
        holder.vcorgname.setText(purchase.getOwner().getVcorgname());
        holder.vcvarietyname.setText(purchase.getSeedId().getVcvarietyname());
        holder.tMysupplierName.setText(purchase.getPohdId().getTMysupplierName());
...
//        String sb = format.format(date);
    ...
    private void update(ViewHolder holder, int position) {
        Intent intent = new Intent(mContext, PurchaseAddActivity.class);
        Purchase.ListBean listBean = list.get(position);
        intent.putExtra("purchase",listBean);
    ...
    private void delete(ViewHolder holder, int position) {
            new AlertDialog.Builder(mContext)
                    ...
                    .setPositiveButton("确定", (dialog, which) -> {
                        ...
                        OkhttpHelper.Get(Constants.PURCHASE_DEL + id, new OkhttpHelper.GetCallBack() {
                            @Override
