...
import agricultural.nxt.agriculturalsupervision.entity.PurPestDetail;
import agricultural.nxt.agriculturalsupervision.entity.PurPesticide;
import butterknife.BindView;
import butterknife.OnClick;
import cn.pedant.SweetAlert.SweetAlertDialog;

public class PurPestAddActivity extends BaseActivity {
    ...
    private PurPesticide.ListBean purchase;
    ...
    protected void initView() {
        ...
        if ("update".equalsIgnoreCase(type)) {
            isUpdate = true;
            purchase = intent.getParcelableExtra("purPest");
        }
        //获取供应商列表
        ...
        //获取农药列表
        OkhttpHelper.Get(Constants.PUR_PESTICIDE_GETPEST, new OkhttpHelper.GetCallBack() {
            @Override
            public void onSuccess(String response, int tag) {
                try {
                    JSONArray array = new JSONArray(response);
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        seedId.add(id);
                        if (isUpdate ) {
                            if (vcvarietyname.equals(purchase.getPesticide().getVcpesticidename())){
                                seedPos = i;
                            }
                        }
                    }
        ...
        if (isUpdate){
            OkhttpHelper.Get(Constants.PUR_PESTICIDE_DETAIL + purchase.getPoh().getId(), new OkhttpHelper.GetCallBack() {
                @Override
                public void onSuccess(String response, int tag) {
                    PurPestDetail detail = new Gson().fromJson(response, PurPestDetail.class);
                    pohremarks.setText(detail.getPopesticided().get(0).getPohremarks());
                    remarks.setText(detail.getPoh().getRemarks());
                }
                @Override
                public void onFailed(String error, int tag) {
        ...
        toolBar.setTitle("update".equalsIgnoreCase(type) ? "农药购进修改" : "农药购进添加");
        if (isUpdate) {
        ...
        OkhttpHelper.Post(Constants.PUR_PESTICIDE_SAVE, map, new OkhttpHelper.PostCallBack() {
            ...
            public void onSuccess(String response, int tag) {
                ...
                if ("true".equalsIgnoreCase(JsonUtil.PareJson(response))) {
                    if (isUpdate) {
                        new SweetAlertDialog(PurPestAddActivity.this, SweetAlertDialog.SUCCESS_TYPE)
                                .setConfirmText("修改成功")
                                .show();
                    } else {
                        new SweetAlertDialog(PurPestAddActivity.this, SweetAlertDialog.SUCCESS_TYPE)
                    ...
                    if (isUpdate) {
                        new SweetAlertDialog(PurPestAddActivity.this, SweetAlertDialog.ERROR_TYPE)
    ...
    protected int getLayoutResId() {
        return R.layout.activity_pur_pest_add;
    }
}
