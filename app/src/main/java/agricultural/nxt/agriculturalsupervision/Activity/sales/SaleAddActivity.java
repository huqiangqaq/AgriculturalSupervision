...
import agricultural.nxt.agriculturalsupervision.entity.SaleDetail;
...
public class SaleAddActivity extends BaseActivity {
    ...
    protected void initView() {
            ...
            public void onSuccess(String response, int tag) {
                try {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        supplierId.add(id);
                        if (isUpdate) {
                            if (vcmysuppliername.equals(sale.getSoh().getMyCustomer())){
                                supplierPos = i;
                            }
                        }
                    }
        ...
        //获取种子列表
        OkhttpHelper.Get(Constants.SALE_GETSEED, new OkhttpHelper.GetCallBack() {
            @Override
            public void onSuccess(String response, int tag) {
                try {
                    JSONArray array = new JSONArray(response);
                    for (int i = 0; i < array.length(); i++) {
                        String vcvarietyname = array.getJSONObject(i).getString("vcvarietyname");
                        String id = array.getJSONObject(i).getString("id");
                        seedList.add(vcvarietyname);
                        seedId.add(id);
                        if (isUpdate ) {
                            if (vcvarietyname.equals(sale.getSeedbatch().getSeed().getVcvarietyname())){
                                seedPos = i;
                            }
                        }
                    }
        ...
        if (isUpdate){
            OkhttpHelper.Get(Constants.SALE_DETAIL + sale.getSoh().getId(), new OkhttpHelper.GetCallBack() {
                @Override
                public void onSuccess(String response, int tag) {
                    SaleDetail detail = new Gson().fromJson(response, SaleDetail.class);
                    pohremarks.setText(detail.getSoseedd().get(0).getSohremarks());
        ...
        toolBar.setTitle("update".equalsIgnoreCase(type) ? "种子销售修改" : "种子销售添加");
        if (isUpdate) {
            btnUpdateAdd.setText("修改");
            dtpodate.setText(sale.getDtsodate());
            fponumber.setText(sale.getFnumber());
            fprice.setText(sale.getFprice());
            ftotalmoney.setText(Double.valueOf(sale.getFnumber()) * Double.valueOf(sale.getFprice()) + "元");
        }
...
//        map.put("tSeedbatchId", seedId.get(tSeedId.getSelectedIndex()));
        ...
        OkhttpHelper.Post(Constants.SALE_SAVE, map, new OkhttpHelper.PostCallBack() {
            ...
            public void onSuccess(String response, int tag) {
                ...
                if ("true".equalsIgnoreCase(JsonUtil.PareJson(response))) {
                    if (isUpdate) {
                        new SweetAlertDialog(SaleAddActivity.this, SweetAlertDialog.SUCCESS_TYPE)
                                .setConfirmText("修改成功")
                                .show();
                    } else {
                        new SweetAlertDialog(SaleAddActivity.this, SweetAlertDialog.SUCCESS_TYPE)
                    ...
                    if (isUpdate) {
                        new SweetAlertDialog(SaleAddActivity.this, SweetAlertDialog.ERROR_TYPE)
