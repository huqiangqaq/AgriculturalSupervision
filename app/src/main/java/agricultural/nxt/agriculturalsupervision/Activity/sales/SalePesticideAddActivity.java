...
import agricultural.nxt.agriculturalsupervision.entity.SalePestDeatil;
import agricultural.nxt.agriculturalsupervision.entity.SalePesticide;
import butterknife.BindView;
import butterknife.OnClick;
import cn.pedant.SweetAlert.SweetAlertDialog;

public class SalePesticideAddActivity extends BaseActivity {
    ...
    Button btnUpdateAdd;
    @BindView(R.id.vcRegName)
    EditText vcRegName;
    @BindView(R.id.vcRegIDCode)
    EditText vcRegIDCode;
    ...
    private SalePesticide.ListBean purchase;
    ...
    protected void initView() {
        ...
        if ("update".equalsIgnoreCase(type)) {
            isUpdate = true;
            purchase = intent.getParcelableExtra("salepesticide");
            seedList.add(purchase.getPesticidebatch().getPesticide().getVcpesticidename());
            seedId.add(purchase.getPesticidebatch().getVcbatchno());
        }
        //获取供应商列表
        ...
        //获取农药列表
        OkhttpHelper.Get(Constants.SALE_PESTICIDE_GETPEST, new OkhttpHelper.GetCallBack() {
            @Override
            public void onSuccess(String response, int tag) {
                try {
                    JSONArray array = new JSONArray(response);
                    String msg = array.getJSONObject(0).getString("mag");
                    boolean flag = msg.contains("记录");
                    if (!flag) {
                        for (int i = 0; i < array.length(); i++) {
                            String vcvarietyname = array.getJSONObject(i).getString("vcpesticidename");
                            String id = array.getJSONObject(i).getString("id");
                            seedList.add(vcvarietyname);
                            seedId.add(id);
                            if (isUpdate) {
                                if (vcvarietyname.equals(purchase.getPesticidebatch().getPesticide().getVcpesticidename())) {
                                    seedPos = i;
                                }
                            }
                        }
        ...
        if (isUpdate) {
            OkhttpHelper.Get(Constants.SALE_PESTICIDE_DETAIL + purchase.getSoh().getId(), new OkhttpHelper.GetCallBack() {
                @Override
                public void onSuccess(String response, int tag) {
                    SalePestDeatil detail = new Gson().fromJson(response, SalePestDeatil.class);
                    pohremarks.setText(detail.getSopesticided().get(0).getSohremarks());
                    remarks.setText(detail.getSoh().getRemarks());
                    vcRegName.setText(detail.getSoh().getVcRegName());
                    vcRegIDCode.setText(detail.getSoh().getVcRegIDCode());
                }
        ...
        toolBar.setTitle("update".equalsIgnoreCase(type) ? "农药销售修改" : "农药销售添加");
        if (isUpdate) {
...
//        map.put("tPesticidebatchId",seedId.get(tSeedId.getSelectedIndex()));
        ...
        OkhttpHelper.Post(Constants.SALE_PESTICIDE_SAVE, map, new OkhttpHelper.PostCallBack() {
            @Override
            public void onSuccess(String response, int tag) {
                dismissLoadingDialog();
                if ("true".equalsIgnoreCase(JsonUtil.PareJson(response))) {
                    if (isUpdate) {
                        new SweetAlertDialog(SalePesticideAddActivity.this, SweetAlertDialog.SUCCESS_TYPE)
                                .setConfirmText("修改成功")
                                .show();
                    } else {
                        new SweetAlertDialog(SalePesticideAddActivity.this, SweetAlertDialog.SUCCESS_TYPE)
                    ...
                    if (isUpdate) {
                        new SweetAlertDialog(SalePesticideAddActivity.this, SweetAlertDialog.ERROR_TYPE)
    ...
    protected int getLayoutResId() {
        return R.layout.activity_sale_pesticide_add;
    }
}
