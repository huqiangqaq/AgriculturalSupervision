...
import agricultural.nxt.agriculturalsupervision.entity.PurchaseDetail;
...
public class PurchaseAddActivity extends BaseActivity {
    ...
    private Purchase.ListBean purchase;
    ...
    protected void initView() {
            ...
            public void onSuccess(String response, int tag) {
                try {
                    ...
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        supplierId.add(id);
                        if (isUpdate) {
                            if (vcmysuppliername.equals(purchase.getPohdId().getTMysupplierName())){
                                supplierPos = i;
                            }
                        }
                    }
        ...
        //获取种子列表
        OkhttpHelper.Get(Constants.PURCHASE_GETSEED, new OkhttpHelper.GetCallBack() {
            @Override
            public void onSuccess(String response, int tag) {
                try {
                    JSONArray array = new JSONArray(response);
                    for (int i = 0; i < array.length(); i++) {
                        ...
                        seedId.add(id);
                        if (isUpdate ) {
                            if (vcvarietyname.equals(purchase.getSeedId().getVcvarietyname())){
                                seedPos = i;
                            }
                        }
                    }
        ...
        if (isUpdate){
            OkhttpHelper.Get(Constants.PURCHASE_DETAIL + purchase.getPohdId().getId(), new OkhttpHelper.GetCallBack() {
                @Override
                public void onSuccess(String response, int tag) {
                    PurchaseDetail detail = new Gson().fromJson(response, PurchaseDetail.class);
                    pohremarks.setText(detail.getPoseedd().get(0).getPohremarks());
        ...
        OkhttpHelper.Post(Constants.PURCHASE_SAVE, map, new OkhttpHelper.PostCallBack() {
            ...
            public void onSuccess(String response, int tag) {
                ...
                if ("true".equalsIgnoreCase(JsonUtil.PareJson(response))) {
                    if (isUpdate) {
                        new SweetAlertDialog(PurchaseAddActivity.this, SweetAlertDialog.SUCCESS_TYPE)
                                .setConfirmText("修改成功")
                                .show();
                    } else {
                        new SweetAlertDialog(PurchaseAddActivity.this, SweetAlertDialog.SUCCESS_TYPE)
                    ...
                    if (isUpdate) {
                        new SweetAlertDialog(PurchaseAddActivity.this, SweetAlertDialog.ERROR_TYPE)
